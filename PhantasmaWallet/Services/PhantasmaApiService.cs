...
namespace Phantasma.Wallet.Services
{
    public class PhantasmaApiService //todo interface
    {
        ...
        {
            _restClient = new HttpClient() { BaseAddress = new Uri("http://localhost:1234/api/") };
        }
        ...
        public async Task<Account> GetAccount(string address)
        {
            try
            {
                HttpResponseMessage responseMessage = await _restClient.GetAsync($"account/{address}");
        ...
        public async Task<Block> GetBlock(string blockHash)
        {
            try
            {
                HttpResponseMessage responseMessage = await _restClient.GetAsync($"block/{blockHash}");
                responseMessage.EnsureSuccessStatusCode();
                var data = await responseMessage.Content.ReadAsStringAsync();
